...
export default {
  async fetch(request) {
    ...
    if (!apiKey) return json({ error: 'The AI Coach is not configured for this deployment.' }, 503);
    ...
    if (!Array.isArray(body.messages) || body.messages.length === 0 || body.messages.length > 16) {
      return json({ error: 'Send between 1 and 16 conversation messages.' }, 400);
    }
    ...
    if (!messages.length || messages.length !== body.messages.length) return json({ error: 'Each message needs a valid role and text content.' }, 400);
...
Help the user understand or use these features, choose one meaningful outcome, break work into a clear next action, plan realistic work blocks, or reflect on attention patterns. Never shame the user, over-optimize their day, invent personal app data, or make medical claims. Prefer a short useful answer over a lecture. Ask at most one question when missing context genuinely prevents useful advice.`;
    ...
    try {
      ...
      return json({ error: 'The coach service could not be reached.' }, 502);
    }

    if (!upstream.ok) {
      const problem = await upstream.json().catch(() => null);
      return json({ error: problem?.error?.message || 'The AI provider rejected the coach request.' }, upstream.status >= 400 && upstream.status < 600 ? upstream.status : 502);
    }
