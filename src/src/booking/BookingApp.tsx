...
import {
    ...
    fetchBookedSlots,
    fetchSchedulingLink,
    fetchSchedulingLinkLocal,
    notifySchedulingBooking,
} from '../lib/schedulingApi';
import { guestFieldsForLocation, validateGuestInput } from '../lib/guestFields';
import { slotsForDay, type BookedSlot } from '../lib/schedulingSlots';
...
export default function BookingApp({ client }: Props = {}) {
    ...
    const loadBooked = useCallback(
        async (activeSlug: string) => {
            ...
            const slots = await fetchBookedSlots(db, activeSlug, from, to);
            setBooked(slots);
        },
        [db],
    );
    ...
    useEffect(() => {
        ...
        (async () => {
            let found = await fetchSchedulingLink(db, slug);
            if (!found) found = await fetchSchedulingLinkLocal(slug);
            if (cancelled) return;
            if (!found) {
                setError(
    ...
    const handleConfirm = async () => {
        ...
        const result = await createSchedulingBooking(db, {
            slug: link.slug,
        ...
        if (result.bookingId) {
            const emailed = await notifySchedulingBooking(db, result.bookingId);
            if (!emailed.ok) {
