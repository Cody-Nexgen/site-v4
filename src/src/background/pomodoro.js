...
import { recordFutureSelfEvent } from './futureSelfService.js';
...
async function processSegmentComplete() {
    ...
    const wasBreak = !!rt.isBreak;
    const completedSegmentId = rt.segmentId;
    const focusMin = rt.focusMin || settings.focusMin || 25;
    const breakMin = rt.breakMin || settings.breakMin || 5;

    // Transition FIRST so the timer never sticks at 00:00 if awards hang.
    if (!wasBreak) {
    ...
    try {
        const isNewCompletion = settings.lastCompletedSegmentId !== completedSegmentId;
        if (isNewCompletion) {
        ...
        await onPomodoroComplete(completedSegmentId, focusMin);
        if (rt.futureSelfContractId) {
            ...
            await recordFutureSelfEvent('break_started', {
                segmentId: `break-${completedSegmentId}`,
            });
        }
    } catch (e) {
        console.warn('[Pomodoro] post-complete awards failed', e);
    }
}
